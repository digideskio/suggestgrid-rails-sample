class Movie < ActiveRecord::Base
  ...
  def create_action(user)
    begin
      ...
      body.userid = user.id
      body.itemid = self.id
      SuggestGrid::ActionController.new.create_action(body, 'movie_space', 'view')
    rescue Exception => e
  ...
  def self.recommend(user, size = 3)
    begin
      ...
      body.size = size
      recommendations = SuggestGrid::RecommendationController.new.recommend_items(body, 'movie_space', 'view')
  ...
  def similar(size = 3)
    begin
      ...
      similars = SuggestGrid::SimilarityController.new.get_similar_items(body, self.id , 'movie_space', 'view')
